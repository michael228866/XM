...
def serialize_metrics(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: serialize_metrics(item) for key, item in value.items()}
    if isinstance(value, list):
        return [serialize_metrics(item) for item in value]
...
def self_check() -> None:
    ...
    assert serialize_metrics([{"value": np.float64(1.0)}]) == [{"value": 1.0}]
