"""Independent one-shot archive verifier; no execution-module imports."""
import argparse
import ast
import calendar
import hashlib
...
def validate(run):
    ...
    checks['script_snapshot']=sha(run/'training_script.py')==m['training_script_sha256']==sha(s/'run_gold_future_capture_final_readiness_v1.py')
    checks['validator_snapshot']=Path(__file__).read_bytes()==(run/'validator_script.py').read_bytes()==(s/'validate_gold_future_capture_final_readiness_v1.py').read_bytes()
    ...
    evidence=load(s/'research_evidence/gold_future_capture_final_readiness_v1/evidence_index.json')
    for item in evidence:
        checks['evidence:'+item['local_path']]=sha(s/item['local_path'])==item['sha256'] and item['authority'] in {'BROKER_DOCUMENT','FEED_DOCUMENT','SIGNED_ATTESTATION','INDEPENDENT_REVIEW'} and all(item.get(k) for k in ('source_url','retrieved_at_utc','reviewed_by','reviewed_at_utc','supported_claims','limitations'))
    adjudication=load(run/'timezone_adjudication.json')
    ...
    diagnostic=load(run/'timezone_transition_diagnostic.json')
    ...
    for sample in diagnostic['samples']:
        ...
        checks['time_sample:'+sample['label']]=len(epochs)<=6 and (end-start).total_seconds()==300 and all(start.timestamp()<=v<=end.timestamp() for v in epochs) and sample['spacing_seconds']==spacing and sample['monotonic']==all(v>0 for v in spacing) and sample['hypothesis_converted']==[legacy_time(v).isoformat() for v in epochs] and sample['offset_inference'] is None and set(sample)=={'label','query_start_utc','query_end_utc','raw_epochs','interpreted_as_utc','hypothesis_converted','spacing_seconds','monotonic','offset_inference','reason'}
    ...
    p=load(run/'prefix_final_readiness.json');tests=load(run/'prefix_equivalence_tests.json');proposal=load(s/'gold_recursive_prefix_reinitialization_proposal_v1.json')
    features=load(s/'execution_spec_gold_independent_secondary_classifier_v1.json')['features']
    tree=ast.parse((s/'drl_trading_v2.py').read_text(encoding='utf-8-sig'))
    fn=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='add_indicators')
    feature_hash=hashlib.sha256(ast.dump(fn,include_attributes=False).encode()).hexdigest()
    checks['feature_identity']=p['pipeline_sha256']==proposal['pipeline_hash']==sha(s/'drl_trading_v2.py') and p['feature_hash']==proposal['feature_hash']==feature_hash and [r['feature_name'] for r in p['feature_inventory']]==features and [r['feature_name'] for r in p['feature_inventory'] if r['recursive']]==['MACD_HIST']
    checks['prefix_honesty']=p['prefix_policy']=='UNRESOLVED' and p['status']=='PARTIAL' and p['approved'] is False and proposal['approved'] is False and proposal['historically_equivalent'] is False and proposal['outcome_tuning'] is False and proposal['exact_source_range'] is None and all(r['earliest_certified_source_utc'] is None for r in p['feature_inventory'])
    checks['prefix_test_hash']=bool(sha(run/'prefix_equivalence_tests.json')) and tests['synthetic_only'] is True and tests['historical_equivalence_tested'] is False and len(tests['tests'])==12
    # Independent fixture recomputation. No historical feature or execution module import.
    import numpy as np
    import pandas as pd
    values=pd.Series(100.0+(i%31)*.125+i/8192 for i in range(4200))
    for row in tests['tests']:
        outputs=[]
        for start in (0,4096-row['candidate_warmup']):
            x=values.iloc[start:];fast=x.ewm(span=12,adjust=False).mean();slow=x.ewm(span=26,adjust=False).mean()
            signal=(fast-slow).ewm(span=9,adjust=False).mean()
            output={'EMA12_STATE':fast,'EMA26_STATE':slow,'MACD_SIGNAL_STATE':signal,'MACD_HIST':fast-slow-signal}[row['feature_name']]
            outputs.append(output.loc[4096:4199].to_numpy())
        difference=np.abs(outputs[0]-outputs[1]);relative=difference/np.maximum(np.abs(outputs[0]),1e-300)
        checks['equivalence:'+row['feature_name']+':'+str(row['candidate_warmup'])]=row['max_abs_difference']==float(difference.max()) and row['max_rel_difference']==float(relative.max()) and row['exact_match']==bool(np.array_equal(*outputs))
    ...
    checks['verdict']=metrics['formal_run_status']==m['formal_run_status']==('FAIL' if expected=='CONTRADICTS_BROKER_SERVER_RULE' else 'PARTIAL')
    ...
    diagnostic_tree=ast.parse((s/'gold_mt5_timezone_transition_diagnostic_v1.py').read_text(encoding='utf-8'))
    ...
    for name in ('gold_mt5_timezone_transition_diagnostic_v1.py','gold_recursive_prefix_final_readiness_v1.py','run_gold_future_capture_final_readiness_v1.py'):
    ...
    checks['commands']=[c['label'] for c in commands]==['self_test','diagnostic','prefix','certification'] and all(c['exit_code']==0 for c in commands)
    selftest=load(run/'self_test_stdout.txt')
    checks['self_tests']=selftest['overall']=='PASS' and len(selftest['checks'])>=36 and selftest['synthetic_only'] is True
    return checks
...
if __name__=='__main__':
    ...
    if args.self_test:
        ...
        with (run/'validator.md').open('x',encoding='utf-8') as f:f.write('# Independent final readiness validation\n\n'+result['overall']+'\nResearch readiness verdict remains separate. No production promotion.\n')
