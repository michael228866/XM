...
OUTPUT_CSV = RESEARCH_DIR / "xaueur_m5_cost_walk_forward.csv"
OUTPUT_JSON = RESEARCH_DIR / "xaueur_m5_cost_walk_forward.json"
OUTPUT_MD = RESEARCH_DIR / "xaueur_m5_cost_walk_forward.md"
...
def slice_by_ratio(frame, start: float, end: float):
    start_idx = int(len(frame) * start)
    end_idx = int(len(frame) * end)
...
def summarize_candidate(rows: list[dict]) -> dict:
    ...
    by_candidate = {}
    for candidate in {row["candidate"] for row in candidate_rows}:
        subset = [row for row in candidate_rows if row["candidate"] == candidate]
        total_r = sum(float(row["test_pnl_r"]) for row in subset)
        total_trades = sum(int(row["test_trades"]) for row in subset)
        pass_count = sum(1 for row in subset if row["fold_pass"])
        positive_count = sum(1 for row in subset if float(row["test_pnl_r"]) > 0)
        weighted_win = (
        ...
        by_candidate[candidate] = {
            ...
            "max_fold_drawdown_r": round(
                min(float(row["test_max_drawdown_r"]) for row in subset), 4
            ),
        }
    return by_candidate
...
def write_outputs(rows: list[dict], summary: dict) -> None:
    ...
    for item in sorted(
        summary.values(), key=lambda value: value["total_pnl_r"], reverse=True
    ):
        lines.append(
            "| {candidate} | {positive_folds}/{folds} | {passed_folds}/{folds} | "
    ...
    for row in rows:
        lines.append(
            "| {fold} | {candidate} | {test_start} -> {test_end} | {test_pnl_r:.2f} | "
            "{test_win_rate:.2%} | {test_profit_factor:.2f} | {test_trades} | "
            "{test_max_drawdown_r:.2f} | {fold_pass} |".format(**row)
        )
...
def main() -> int:
    ...
    for item in sorted(summary.values(), key=lambda value: value["total_pnl_r"], reverse=True):
        print(
            f"{item['candidate']}: total_r={item['total_pnl_r']:.2f}, "
